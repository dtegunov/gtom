...
TEST(Resolution, LocalFSCBfac)
{
	...
	//Case 1:
	{
		...
		HeaderMRC header = ReadMRCHeader("G:\\rescomparison\\run1_half1_class001_unfil.mrc");
		...
		ReadMRC("G:\\rescomparison\\run1_half1_class001_unfil.mrc", &h_mrcraw1);
		ReadMRC("G:\\rescomparison\\run1_half2_class001_unfil.mrc", &h_mrcraw2);
		...
		float Pixelsize = header.pixelsize.x / header.dimensions.x;

		d_LocalFSCBfac(d_input1, d_input2, dimsvolume, d_resolution, d_bfac, d_corrected, d_unsharpened, windowsize, 0.3f, false, 0, 10, Pixelsize, 0, 0, 0, false, false); // -1.34f
		...
		d_WriteMRC(d_resolution, dimsvolume, "G:\\rescomparison\\run1global_localres.mrc");
		//d_WriteMRC(d_bfac, dimsvolume, "F:\\stefanribo\\vlion\\MiniWarpedLocalOST\\bee\\bfac_aniso.mrc");
		d_WriteMRC(d_corrected, dimsvolume, "G:\\rescomparison\\run1global_localfilt.mrc");

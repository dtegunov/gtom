...
TEST(Optimization, OptimizeTomoParams)
{
	...
	//Case 1:
	{
		...
		string imagename = "L3Tomo3.soft.256.mrc";
		string paramnameroot = "L3Tomo3.256";
		...
		//int3 dimsvolume = toInt3(dimsimage.x, dimsimage.y, dimsimage.x * 5 / 16);
		...
		tfloat3 deltaangle = tfloat3(ToRad(1e-5f), ToRad(1e-5f), ToRad(10.0f));
		...
		// Define groups
		vector<tfloat2> groupranges;
		groupranges.push_back(tfloat2(ToRad(-90.0), ToRad(-29.0)));
		groupranges.push_back(tfloat2(ToRad(-51.0), ToRad(51.0)));
		groupranges.push_back(tfloat2(ToRad(29.0), ToRad(90.0)));

		vector<vector<int>> indicesgroups;
		for (uint g = 0; g < groupranges.size(); g++)
			indicesgroups.push_back(vector<int>());

		for (int n = 0; n < nimages; n++)
			for (int g = 0; g < groupranges.size(); g++)
				if (h_angles[n].y >= groupranges[g].x && h_angles[n].y <= groupranges[g].y)
					indicesgroups[g].push_back(n);

		vector<int3> dimsvolume;
		dimsvolume.push_back(toInt3(dimsimage.x * 2, dimsimage.y, dimsimage.x * 5 / 16));
		dimsvolume.push_back(toInt3(dimsimage.x, dimsimage.y, dimsimage.x * 5 / 16));
		dimsvolume.push_back(toInt3(dimsimage.x * 2, dimsimage.y, dimsimage.x * 5 / 16));

...
TEST(Reductions, Max)
{
	...
	{		
		...
		tuple2<tfloat, size_t> cpuResult(INT_MIN, 0);
		tfloat* h_input = MallocValueFilled<tfloat>(size, 1);
		for(int j = 0; j < size; j++)
			h_input[j] = (tfloat)(rand() % 100000) * 0.00001f - 0.5f;
		h_input[rand() % size] = 0.500001f;
		for(int j = 0; j < size; j++)
		...
		CUDA_MEASURE_TIME(d_Max(d_input, d_output, size, 1));

...
namespace gtom
{
	...
	void Picker::Initialize(tfloat* _d_refs, int3 _dimsref, uint _nrefs, tfloat* _d_refmasks, bool _ismaskcircular, bool _doctf, int3 _dimsimage, uint _lowpassfreq)
	{
		dimsref = _dimsref;
		nrefs = _nrefs;
	...
	void Picker::PerformCorrelation(uint n, tfloat anglestep, tfloat* d_bestccf, tfloat3* d_bestangle, int* d_bestref)
	{
		...
		for (uint a = 0; a < angles.size(); a++)
		{
			...
			PickingUpdateKernel <<<grid, TpB>>> (d_buffer1, 
	...
	void Picker::CalcSolventStatistics(tcomplex* d_imageft, tcomplex* d_image2ft, tcomplex* d_solventmaskft, tfloat solventsamples, tfloat* d_solventmean, tfloat* d_solventstd)
	{
		tcomplex* d_buffer;
		...
		int TpB = 256;
		dim3 grid = dim3(tmin((Elements(dimsimage) + TpB - 1) / TpB, 16384), 1, 1);
		SolventStatisticsKernel << <grid, TpB >> > (d_solventmean, d_solventstd, (tfloat)1 / ((tfloat)Elements(dimsimage) * solventsamples), Elements(dimsimage));

			   ...
			   T_ALIGN_MODE mode)
{
	if(mode == T_ALIGN_BOTH)
	{
		...
		cudaMalloc((void**)&d_normtargets, ElementsFFT(dims) * sizeof(tcomplex));

		tfloat* d_scaled;
		cudaMalloc((void**)&d_scaled, Elements(dims) * sizeof(tfloat));

		tfloat* d_scores = CudaMallocValueFilled(Elements(dims), (tfloat)-1);
		tfloat* d_allrotationsx = CudaMallocValueFilled(Elements(dims), (tfloat)0);
		tfloat* d_allrotationsy = CudaMallocValueFilled(Elements(dims), (tfloat)0);
		tfloat* d_allrotationsz = CudaMallocValueFilled(Elements(dims), (tfloat)0);
		
		{
			...
			long memory = 3 * 1024 * 1024 * 1024;
			long memperprimitive = Elements(dimsscaled) * 2 * sizeof(tfloat) + ElementsFFT(dimsscaled) * sizeof(tcomplex);
			...
			if(pixelsperstep > 1)
			{
				d_Scale(d_input, d_scaled, dims, dimsscaled, T_INTERP_FOURIER);
				d_Scale(d_normtargets, d_normtargets, dims, dimsscaled, T_INTERP_FOURIER, NULL, NULL, numtargets);
				d_Norm(d_normtargets, d_normtargets, Elements(dimsscaled), (char*)NULL, T_NORM_MEAN01STD, 0, numtargets);
			}
			else
			{
				cudaMemcpy(d_scaled, d_input, Elements(dims) * sizeof(tfloat), cudaMemcpyDeviceToDevice);
				d_Norm(d_normtargets, d_normtargets, Elements(dims), (char*)NULL, T_NORM_MEAN01STD, 0, numtargets);
			}
		...
		for (int i = 0; i < rotationrefinements; i++)
		{
			...
			for(int t = 0; t < numtargets; t++)
			{
				...
				for (int a = 0; a < numangles; a += batchsize)
				{
					...
					d_Rotate3D(d_input, d_rotated, dimsscaled, h_angles + a, T_INTERP_LINEAR, currentbatch);
		...
		cudaFree(d_allrotationsz);
		cudaFree(d_allrotationsy);
		cudaFree(d_allrotationsx);
		cudaFree(d_scores);
		cudaFree(d_scaled);
	...
	else if(mode == T_ALIGN_ROT)
	{
		...
		{
			tfloat3 negposition = tfloat3(-position.x, -position.y, -position.z);
			d_Shift(d_targets, d_normtargets, dims, &negposition, NULL, NULL, NULL, numtargets);
		}
		else
			cudaMemcpy(d_normtargets, d_targets, Elements(dims) * numtargets * sizeof(tfloat), cudaMemcpyDeviceToDevice);

...
namespace gtom
{
	...
	void d_rlnBackproject(tcomplex* d_volumeft, tfloat* d_volumeweights, int3 dimsvolume, tcomplex* d_projft, tfloat* d_projweights, int3 dimsproj, uint rmax, glm::mat3* d_matrices, uint batch)
	{
		...
		if (ndimsvolume == 3)
		{
			...
			{
				if (ndimsproj == 2)
					Backproject3DtoNDKernel<2, 32> << <grid, 32 >> > (d_volumeft, d_volumeweights, dimsvolume.x, d_projft, d_projweights, dimsproj.x, elements, rmax, rmax * rmax);
				else if (ndimsproj == 3)
					Backproject3DtoNDKernel<3, 32> << <grid, 32 >> > (d_volumeft, d_volumeweights, dimsvolume.x, d_projft, d_projweights, dimsproj.x, elements, rmax, rmax * rmax);
			}
			else if (TpB == 64)
			{
				if (ndimsproj == 2)
					Backproject3DtoNDKernel<2, 64> << <grid, 64 >> > (d_volumeft, d_volumeweights, dimsvolume.x, d_projft, d_projweights, dimsproj.x, elements, rmax, rmax * rmax);
				else if (ndimsproj == 3)
					Backproject3DtoNDKernel<3, 64> << <grid, 64 >> > (d_volumeft, d_volumeweights, dimsvolume.x, d_projft, d_projweights, dimsproj.x, elements, rmax, rmax * rmax);
			}
			else if (TpB == 128)
			{
				if (ndimsproj == 2)
					Backproject3DtoNDKernel<2, 128> << <grid, 128 >> > (d_volumeft, d_volumeweights, dimsvolume.x, d_projft, d_projweights, dimsproj.x, elements, rmax, rmax * rmax);
				else if (ndimsproj == 3)
					Backproject3DtoNDKernel<3, 128> << <grid, 128 >> > (d_volumeft, d_volumeweights, dimsvolume.x, d_projft, d_projweights, dimsproj.x, elements, rmax, rmax * rmax);
			}
			else if (TpB == 256)
			{
				if (ndimsproj == 2)
					Backproject3DtoNDKernel<2, 256> << <grid, 256 >> > (d_volumeft, d_volumeweights, dimsvolume.x, d_projft, d_projweights, dimsproj.x, elements, rmax, rmax * rmax);
				else if (ndimsproj == 3)
					Backproject3DtoNDKernel<3, 256> << <grid, 256 >> > (d_volumeft, d_volumeweights, dimsvolume.x, d_projft, d_projweights, dimsproj.x, elements, rmax, rmax * rmax);
			}
			else
				throw;
		}

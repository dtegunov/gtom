...
namespace gtom
{
	...
	template<bool amplitudesquared> __global__ void CTFSimulateKernel(half* d_output, half2* d_addresses, int n, CTFParamsLean* d_p);
	...
	void d_CTFSimulate(CTFParams* h_params, half2* d_addresses, half* d_output, uint n, bool amplitudesquared, int batch)
	{
		CTFParamsLean* h_lean;
		cudaMallocHost((void**)&h_lean, batch * sizeof(CTFParamsLean));
		#pragma omp parallel for

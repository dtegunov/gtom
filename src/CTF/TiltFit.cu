...
namespace gtom
{
	...
	void PopulateAngles(std::vector<tfloat3> &v_angles, tfloat3 phibracket, tfloat3 thetabracket, tfloat3 psibracket)
	{
	...
	void h_CTFTiltFit(tfloat* h_image, int2 dimsimage, uint nimages, float overlapfraction, std::vector<CTFTiltParams> &startparams, CTFFitParams fp, tfloat maxtheta, tfloat2 &specimentilt, tfloat* h_defoci)
	{
		...
		for (uint r = 0; r < 4; r++)
		{
			...
			for (uint i = 0; i < nimages; i++)
			{
				...
				std::sort(v_results.begin(), v_results.end(),
...

		{
			for (int a = 0; a < v_angles.size(); a++)
			{
				...
				for (uint d = 0; d <= defocusrefinements; d++)
				{
					...
					{
						std::vector<std::pair<tfloat, CTFParams> > v_newparams;

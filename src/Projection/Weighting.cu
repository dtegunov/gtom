...
template <bool iscentered, class T> __global__ void Exact2DWeightingKernel(T* d_weights, int2 dims, int index, glm::vec3* d_normals, glm::mat3x2* d_globalB2localB, int nimages, tfloat maxfreq);
...
template <class T> void d_Exact2DWeighting(T* d_weights, int2 dimsimage, int* h_indices, tfloat3* h_angles, int nimages, tfloat maxfreq, bool iszerocentered, int batch)
{
	...
	{
		glm::mat3 tB = Matrix3Euler(tfloat3(h_angles[i].x, h_angles[i].y, h_angles[i].z));
	...
	for (int b = 0; b < batch; b++)
		if (iszerocentered)
			Exact2DWeightingKernel<true, T> <<<grid, TpB>>> (d_weights + (dimsimage.x / 2 + 1) * dimsimage.y * b, dimsimage, h_indices[b], d_normals, d_globalB2localB, nimages, maxfreq);
		else
			Exact2DWeightingKernel<false, T> <<<grid, TpB>>> (d_weights + (dimsimage.x / 2 + 1) * dimsimage.y * b, dimsimage, h_indices[b], d_normals, d_globalB2localB, nimages, maxfreq);

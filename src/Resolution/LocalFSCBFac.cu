...
namespace gtom
{
	...
	template<bool anisotropic> __global__ void FSCBFacKernel(tcomplex* d_volume1, tcomplex* d_volume2, uint sidelength, uint sidelengthft, tfloat fscthreshold, uint minbfacshell, tfloat angpix, tfloat minbfac, tfloat bfacbias, tfloat mtfslope, tfloat* d_resolution, tfloat* d_bfactors, tfloat* d_debugfsc);
						...
						bool doanisotropy)
	{
		...
		for (int i = 0; i < elementsvol; i += batchsize)
		{
			...
			if (doanisotropy)
				FSCBFacKernel<true> << <grid, TpB >> > (d_extractsft1, 
														d_extractsft2, 
														windowsize, 
														windowsize / 2 + 1, 
														fscthreshold, 
				...
				FSCBFacKernel<false> << <grid, TpB >> > (d_extractsft1, 
														d_extractsft2, 
														windowsize, 
														windowsize / 2 + 1, 
														fscthreshold, 
	...
	template<bool anisotropic> __global__ void FSCBFacKernel(tcomplex* d_volume1, tcomplex* d_volume2, uint sidelength, uint sidelengthft, tfloat fscthreshold, uint minbfacshell, tfloat angpix, tfloat minbfac, tfloat bfacbias, tfloat mtfslope, tfloat* d_resolution, tfloat* d_bfactors, tfloat* d_debugfsc)
	{
		...
		for (uint axisid = 0; axisid < naxes; axisid++)
		{
			...
			if (threadIdx.x < 64)
			{
				...
				fscweights[axisid][threadIdx.x] = tmin(sqrt((tfloat)2 * nums[threadIdx.x] / (nums[threadIdx.x] + (tfloat)1)), 1);
		...
		// Filter and sharpen the sum of the two half-maps
		{
			uint sidelengthhalf = sidelength / 2;
			...
			for (uint id = threadIdx.x; id < elementscube; id += 128)
			{
				...
				tcomplex val = d_volume1[id] + d_volume2[id];
				if (ri == 0)
				{
					d_volume2[id] = val;
					d_volume1[id] = val;
	...
	/*__global__ void FSCBFacKernel(tcomplex* d_volume1, tcomplex* d_volume2, uint sidelength, uint sidelengthft, tfloat fscthreshold, uint minbfacshell, tfloat angpix, tfloat* d_resolution, tfloat* d_bfactors, tfloat* d_debugfsc)
	{
		__shared__ tfloat nums[64];
		__shared__ tfloat denoms1[64];
		__shared__ tfloat denoms2[64];
		...
		__shared__ tfloat fscweights[3][64];

		__shared__ tfloat finalres[3];
		__shared__ tfloat finalbfac[3];
		...
		nums[threadIdx.x] /= tmax(1e-20f, sqrt(denoms1[threadIdx.x] * denoms2[threadIdx.x]));					// Calculate FSC
		nums[threadIdx.x] = tmax(1e-20f, nums[threadIdx.x]);
		denoms2[threadIdx.x] = tmin(sqrt((tfloat)2 * nums[threadIdx.x] / (nums[threadIdx.x] + (tfloat)1)), 1);
		amps[threadIdx.x] *= denoms2[threadIdx.x];																// Weight amplitudes by sqrt(2 * FSC / (FSC + 1))
		//amps[threadIdx.x] /= 1.0f + (float)threadIdx.x / sidelength * (-1.393f);								// Divide by MTF
		...
		if (threadIdx.x == 0)
		{
			...
			finalres = tmax(1, (tfloat)i + tmax(tmin((fscthreshold - current) / (next - current + (tfloat)0.00001), 1.0f), 0.0f));
			//finalres = tmin(11.0, finalres);
			d_resolution[blockIdx.x] = finalres;
		...
		//amps[threadIdx.x] = threadIdx.x < maxresbfac ? amps[threadIdx.x] : 0;
		if (threadIdx.x >= minbfacshell && threadIdx.x < maxresbfac)
		{
			tfloat res = tmax(1e-6f, (sidelength * angpix) / threadIdx.x);
			nums[threadIdx.x] = tmin(999, 1 / (res * res));
		...
		if (threadIdx.x == 0)
		{
			...
			if (ss_xx > 0)
				finalbfac = 4 * ss_xy / ss_xx;
			else
				finalbfac = 0;

			//finalbfac = tmax(-90, finalbfac);
			//finalbfac = -80;

			if (isnan(finalbfac))
				finalbfac = 0;

			finalbfac = tmin(tmax(-600, finalbfac + 0), 0);
			//finalbfac = -140;
			//finalbfac = 0;

			d_bfactors[blockIdx.x] = finalbfac;
		}

		__syncthreads();

		{
			tfloat res = threadIdx.x / (sidelength * angpix);
			denoms1[threadIdx.x] = exp(finalbfac * 0.25f * res * res);
			//denoms1[threadIdx.x] *= 1.0f + (float)threadIdx.x / sidelength * (-1.393f);

			//if (threadIdx.x < sidelengthhalf)
				//d_debugfsc[threadIdx.x] = denoms1[threadIdx.x];
		}
		...
		for (uint id = threadIdx.x; id < elementscube; id += 128)
		{
			...
			tfloat rz = idz <= sidelengthhalf ? idz : idz - (int)sidelength;
			tfloat radius = sqrt(rx * rx + ry * ry + rz * rz);
			uint ri = tmin((uint)(radius + 0.5f), sidelength - 1);

			tcomplex val = d_volume1[id] + d_volume2[id];

			tfloat fscweight = 1 - tmax(0, tmin(1, radius - finalres));
			val *= fscweight;
			val *= denoms2[ri];

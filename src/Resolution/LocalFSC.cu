...
namespace gtom
{
					...
					tfloat* h_globalfsc)
	{
		...
		for (int i = 0; i < elementssparse; i += batchsize)
		{
			...
			// Extract windows
			d_ExtractMany(d_volume1, d_extracts1, dimsvolume, dimswindow, d_sparseorigins + i, curbatch);
			d_ExtractMany(d_volume2, d_extracts2, dimsvolume, dimswindow, d_sparseorigins + i, curbatch);

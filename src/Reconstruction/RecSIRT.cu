...
template <bool cubicinterp> __global__ void RaytraceVolumeKernel(cudaTextureObject_t t_volume, int3 dimsvolume, tfloat* d_image, tfloat* d_projection, int2 dimsimage, float* d_distmin, float* d_distmax, glm::vec3 direction, glm::mat4 transform);
template <bool cubicinterp> __global__ void CorrectionKernel(tfloat* d_volume, int3 dimsvolume, cudaTextureObject_t* t_image, int2 dimsimage, int nimages, glm::mat4* d_transform);
template <bool cubicinterp> __global__ void ResidualKernel(tfloat* d_residual, int3 dimsvolume, cudaTextureObject_t* t_image, int2 dimsimage, int nimages, glm::mat4* d_transform);
...
void d_RecSIRT(tfloat* d_volume, tfloat* d_residual, int3 dimsvolume, tfloat3 offsetfromcenter, tfloat* d_image, int2 dimsimage, int nimages, tfloat3* h_angles, tfloat2* h_offsets, tfloat2* h_scales, T_INTERP_MODE mode, int supersample, int iterations, bool outputzerocentered)
{
	...
	for (int i = 0; i < iterations; i++)
	{
		...
		dim3 TpB = dim3(16, 16);
		dim3 grid = dim3((dimsimage.x + 15) / 16, (dimsimage.y + 15) / 16);
		for (int n = 0; n < nimages; n++)
		{
			glm::vec3 direction = Matrix3Euler(h_angles[n]) * glm::vec3(0.0f, 0.0f, -1.0f);
			if (mode == T_INTERP_CUBIC)
				RaytraceVolumeKernel<true> <<<grid, TpB>>> (t_volume, 
															dimsvolume, 
															d_superimage + Elements2(dimsimage) * n,
															d_reprojections + Elements2(dimsimage) * n,
															dimsimage, 
															d_distmin + Elements2(dimsimage) * n,
															d_distmax + Elements2(dimsimage) * n,
															direction,
															h_raytransforms[n]);
			else
		...
		{
			cudaArray** ha_image = (cudaArray**)malloc(nimages * sizeof(cudaArray*));
			cudaTextureObject_t* ht_image = (cudaTextureObject_t*)malloc(nimages * sizeof(cudaTextureObject_t));
			tfloat* d_temp;
			cudaMalloc((void**)&d_temp, Elements2(dimsimage) * sizeof(tfloat));

			for (int n = 0; n < nimages; n++)
			{
				if (mode == T_INTERP_CUBIC)
				{
					cudaMemcpy(d_temp, d_reprojections + Elements2(dimsimage) * n, Elements2(dimsimage) * sizeof(tfloat), cudaMemcpyDeviceToDevice);
					d_CubicBSplinePrefilter2D(d_temp, dimsimage.x * sizeof(tfloat), dimsimage);
					d_BindTextureToArray(d_temp, ha_image[n], ht_image[n], dimsimage, cudaFilterModeLinear, false);
				}
				else
					d_BindTextureToArray(d_reprojections + Elements2(dimsimage) * n, ha_image[n], ht_image[n], dimsimage, cudaFilterModeLinear, false);
			}
			cudaTextureObject_t* dt_image = (cudaTextureObject_t*)CudaMallocFromHostArray(ht_image, nimages * sizeof(cudaTextureObject_t));
			...
			{
				CorrectionKernel<false> << <grid, TpB >> > (d_supervolume, dimsvolume, dt_image, dimsimage, nimages, d_transforms);
				if (i == iterations - 1 && d_superresidual != NULL)
					ResidualKernel<false> <<<grid, TpB>>> (d_residual, dimsvolume, dt_image, dimsimage, nimages, d_transforms);
			}
			else
			{
				CorrectionKernel<true> << <grid, TpB >> > (d_supervolume, dimsvolume, dt_image, dimsimage, nimages, d_transforms);
				if (i == iterations - 1 && d_superresidual != NULL)
					ResidualKernel<true> << <grid, TpB >> > (d_residual, dimsvolume, dt_image, dimsimage, nimages, d_transforms);
			}
			...
			//CudaWriteToBinaryFile("d_prefilteredvolume.bin", d_prefilteredvolume, Elements(dimsvolume) * sizeof(tfloat));
			//CudaWriteToBinaryFile("d_samples.bin", d_samples, Elements(dimsvolume) * sizeof(tfloat));
...
template <bool cubicinterp> __global__ void CorrectionKernel(tfloat* d_volume, int3 dimsvolume, cudaTextureObject_t* t_image, int2 dimsimage, int nimages, glm::mat4* d_transform)
{
	...
	for (int n = 0; n < nimages; n++)
	{
		...
		if (position.x < 0.0f || position.x > dimsimage.x || position.y < 0.0f || position.y > dimsimage.y)
			continue;
	...
	d_volume[(outz * dimsvolume.y + outy) * dimsvolume.x + outx] += samples > 0 ? sum / (tfloat)samples : 0.0f;
}
...
template <bool cubicinterp> __global__ void ResidualKernel(tfloat* d_residual, int3 dimsvolume, cudaTextureObject_t* t_image, int2 dimsimage, int nimages, glm::mat4* d_transform)
{
	...
	d_residual[(outz * dimsvolume.y + outy) * dimsvolume.x + outx] = sum;
}